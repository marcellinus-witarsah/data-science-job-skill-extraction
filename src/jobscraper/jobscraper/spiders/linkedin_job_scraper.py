...
class LinkedinJobScraperSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for job in jobs:
            job_detail_url = job.css(".base-card__full-link::attr(href)").get(default="not-found")
            job_title = job.css("h3.base-search-card__title::text").get(default="not-found")
            job_listed = job.css("time::attr(datetime)").get(default="not-found")
            company_name = job.css("h4 a::text").get(default="not-found")
            company_location = job.css(".job-search-card__location::text").get(default="not-found")
            yield response.follow(url=job_detail_url, callback=self.parse_jobs_page, meta={
                'job_title': job_title, 
